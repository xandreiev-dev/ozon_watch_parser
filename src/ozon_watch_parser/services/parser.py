...
class OzonWatchParser:
    ...
    async def parse_brand(
        ...
        async def handle_new_items(items: list[ListingItem]) -> None:
            ...
            for item in items:
                if register(item):
                    rows.append(normalize_listing_item(item, brand_hint=brand))
            if rows:
        ...
        rows = [normalize_listing_item(item, brand_hint=brand) for item in collected.values()]
    ...
    async def run_once(
        ...
        try:
            ...
            for brand in selected_brands:
                base_url = url_override or BRAND_URLS[brand]
                required_cards = max(min_cards, BRAND_MIN_CARDS.get(brand, DEFAULT_MIN_CARDS))
                ...
                results[brand] = len(frame)
                if len(frame) < required_cards:
                    logger.warning("[%s] Собрано %s из ожидаемых %s", brand, len(frame), required_cards)
        finally:
            await self.close()
